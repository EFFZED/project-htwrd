    ...
    if request.method == "POST":
        ...
        # <---------------------------------------->
            ...
            if (CHECK_DIS == [0]) & (CHECK_DIT == [0]):
                ...
                for i in range(len(NEW_COLUMN_NAME)):
                    if NEW_DATA_TYPE[i] == 'DATE':
                        IDT_INSERT_TAR = IDT_INSERT_TAR + " START_DATE , END_DATE "
                        IDT_MINMAX_TAR = IDT_MINMAX_TAR + " MIN(" + NEW_COLUMN_NAME[i] + ") as START_DATE, MAX(" + NEW_COLUMN_NAME[i] + ") as END_DATE "
                    else:
                        IDT_INSERT_TAR = IDT_INSERT_TAR + " " + NEW_COLUMN_NAME[i] + " "
                ...
                for i in range(len(PICK_COLUMNS)):
                    ...
                    if i < len(PICK_COLUMNS) - 1:
                        IDT_SELECT_SOU += ","
                    if ((i < len(PICK_COLUMNS) - 1) & (NEW_DATA_TYPE[i] != 'DATE')) & \
                            ~((i == len(PICK_COLUMNS) - 2) & (NEW_DATA_TYPE[len(PICK_COLUMNS) - 1] == 'DATE')):
                        IDT_GROUPBY_SOU += ","

                INSERT_DIM_TABLE = " INSERT INTO " + TAR_USER.upper() + "." + TAR_TABLE_NAME + " " \
                                   " (SRG_KEY, " + IDT_INSERT_TAR + ") " \
                                   " (SELECT " + TAR_USER.upper() + ".SEQ_" + TAR_TABLE_NAME + ".nextval as \"SRG_KEY\"," \
                                   " " + IDT_INSERT_TAR + " FROM (SELECT " + IDT_SELECT_SOU + "" + IDT_MINMAX_TAR + " " \
                                   " FROM " + SOU_USER.upper() + "." + SOU_TABLE_NAME + " GROUP BY " + IDT_GROUPBY_SOU + ")) "
                print(INSERT_DIM_TABLE)
